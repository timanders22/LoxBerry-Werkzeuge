#!/usr/bin/env python3
"""Einzelpruefungen der Abbildungsfunktionen des Volkswagen-Dienstes.
...
Was das NICHT belegt: ob die Volkswagen-Schnittstelle diese Felder bei einem
echten Fahrzeug auch fuellt. Dafuer braucht es ein Konto und ein Auto.

Aufruf:  vw_funktionstest.py [pfad/zu/bin/vw.py]
"""
...
def id4():
    ...
    v.manufacturer._set_value("Volkswagen")
...
# ---------------------------------------------------------------------------
A = vw.fahrzeug_abbilden(id4(), CFG, 0, {})
pruefe("ok", A["ok"], 1)
pruefe("keine Ausfaelle", A["ausfaelle"], {})
pruefe("vin", A["vin"], "WVWZZZE1ZPP000001")
...
B = vw.fahrzeug_abbilden(v, CFG, 0, {})
...
# ---------------------------------------------------------------------------
leer = ElectricVehicle(vin="WVWZZZE1ZPP000002", garage=GARAGE)
C = vw.fahrzeug_abbilden(leer, CFG, 0, {})
...
# ---------------------------------------------------------------------------
h = HybridVehicle(vin="WVWZZZ3CZPE000003", garage=GARAGE)
...
D = vw.fahrzeug_abbilden(h, CFG, 0, {})
...
for zustand, zahl in ((GenericVehicle.State.OFFLINE, 0), (GenericVehicle.State.PARKED, 1),
    ...
    pruefe("Zustand %s" % zustand.value, vw.abbild_status(v)["zustand"], zahl)
for zustand, zahl in ((Doors.LockState.LOCKED, 1), (Doors.LockState.UNLOCKED, 0),
    ...
    pruefe("Verriegelung %s" % zustand.value, vw.abbild_status(v)["verriegelt"], zahl)
for zustand, zahl in ((Doors.OpenState.CLOSED, 0), (Doors.OpenState.OPEN, 1),
    ...
    pruefe("Tueren %s" % zustand.value, vw.abbild_status(v)["tueren_offen"], zahl)
for zustand, zahl in ((Charging.ChargingState.CHARGING, 1), (Charging.ChargingState.OFF, 0),
    ...
    pruefe("Laden %s" % zustand.value, vw.abbild_laden(v)["laedt"], zahl)
for zustand, zahl in ((Climatization.ClimatizationState.OFF, 0),
    ...
    pruefe("Klima %s" % zustand.value, vw.abbild_status(v)["klima_an"], zahl)
...
pruefe("abgeschaltetes Attribut -> None", vw.abbild_stamm(v)["kilometerstand"], None)
...
pruefe("ueberfaellige Inspektion", vw.abbild_wartung(v)["inspektion_tage"], -12)

# ---------------------------------------------------------------------------
# Hilfsfunktionen
# ---------------------------------------------------------------------------
pruefe("wert_zahl None", vw.wert_zahl(None), None)
pruefe("wert_zahl leer", vw.wert_zahl(""), None)
pruefe("wert_zahl Text", vw.wert_zahl("warm"), None)
pruefe("wert_zahl 21", vw.wert_zahl("21"), 21)
pruefe("wert_zahl 21,5", vw.wert_zahl("21,5"), 21.5)
pruefe("wert(None)", vw.wert(None), None)
pruefe("etext(None)", vw.etext(None), "")
pruefe("kennzahl unbekannt", vw.kennzahl(None, vw.VERRIEGELT), None)

liste = [id4()]
pruefe("Fahrzeugwahl per Nummer", vw.fahrzeug_waehlen(liste, "1") is liste[0], True)
pruefe("Fahrzeugwahl per VIN", vw.fahrzeug_waehlen(liste, "wvwzzze1zpp000001") is liste[0], True)
pruefe("Fahrzeugwahl zu gross", vw.fahrzeug_waehlen(liste, "5"), None)
pruefe("Fahrzeugwahl leere Liste", vw.fahrzeug_waehlen([], "1"), None)
...
pruefe("unlesbare Konfiguration -> Vorgaben", vw.config()["intervall"], 300)

# ---------------------------------------------------------------------------
# Die Konfiguration fuer die Bibliothek muss die Untergrenze einhalten
# ---------------------------------------------------------------------------
bc = vw.bibliothek_config({"email": "a@b.de", "passwort": "x", "spin": ""},
                          dict(vw.VORGABEN, intervall=30))
...
bc = vw.bibliothek_config({"email": "a@b.de", "passwort": "x", "spin": "1234"},
                          dict(vw.VORGABEN, zugriff_erzwingen=1))
pruefe("mit S-PIN", bc["carConnectivity"]["connectors"][0]["config"]["spin"], "1234")
pruefe("Zugriff erzwingen", bc["carConnectivity"]["connectors"][0]["config"]["force_enable_access"], True)
...
try:
    CarConnectivity(config=vw.bibliothek_config(
        {"email": "a@b.de", "passwort": "x", "spin": ""}, vw.VORGABEN))
