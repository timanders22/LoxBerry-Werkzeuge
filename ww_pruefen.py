#!/usr/bin/env python3
"""Pflichtpruefung fuer das Weissware-Plugin.
...
Aufruf:  ww_pruefen.py <pluginordner>
"""
...
def pruefe_sprache(wurzel):
    ...
    for f in sorted(wurzel.rglob("*.php")):
        t = f.read_text(encoding="utf-8")
        benutzt |= set(re.findall(r"ww_t\('([A-Z0-9_]+\.[A-Z0-9_]+)'\)", t))
        benutzt |= set(re.findall(r"'(WW_[A-Z]+\.[A-Z0-9_]+|BAUSTEIN\.[A-Z0-9_]+)'", t))
        maskiert |= set(re.findall(r"ww_e\(\s*ww_t\('([A-Z0-9_]+\.[A-Z0-9_]+)'\)", t))
        maskiert |= set(re.findall(r"ww_e\(\s*sprintf\(\s*ww_t\('([A-Z0-9_]+\.[A-Z0-9_]+)'\)", t))
    ...
    for k in sorted(maskiert):
        for name, tab in (("de", tde), ("en", ten)):
            w = tab.get(k, "")
            if "<" in w or "&" in w:
                fehl("%s: %s laeuft durch ww_e(), enthaelt aber Auszeichnung oder "
...
def pruefe_endpunkt(wurzel):
    ...
    muster = dict(re.findall(r"ww_param\('([a-z]+)',\s*'(/[^']+/)'", t))
    erwartet = {"geraet", "programm"}
    if set(muster) != erwartet:
        fehl("Geprueft werden %s, erwartet %s" % (sorted(muster), sorted(erwartet)))

    # Die Kennungen der drei Anbieter enthalten Bindestriche und Punkte und
    # sind unterschiedlich lang; ein enges Zahlenmuster wie bei den
    # Fahrzeug-Plugins geht hier nicht. Geprueft wird deshalb gegen die
    # Eingaben, die wirklich gefaehrlich sind - Schrott wie "abc" fuehrt nur
    # dazu, dass kein Geraet gefunden wird, und das ist eine saubere Antwort.
    boese = ["; rm -rf /", "../../etc/passwd", "1 OR 1=1", "<script>", "%00",
             "1;reboot", "'", '"', "\n", " ", "a/b", "..", "$(id)", "`id`",
             "a" * 100]
    gut = {"geraet": ["1", "12", "SIEMENS-HCS03WCH1-456227783F78", "000160102030",
                      "abc-123-def"],
           "programm": ["LaundryCare.Washer.Program.Cotton",
    ...
    lesend = re.search(r"\$ww_lesend = array\(([^)]*)\)", t)
    schaltend = re.search(r"\$ww_schaltend = array\(([^;]*?)\);", t, re.S)
...
def pruefe_vorlage(wurzel):
    t = (wurzel / "webfrontend/html/ww_lib.php").read_text(encoding="utf-8")
    block = re.search(r"function ww_xml_virtual_in_http.*?\n}", t, re.S)
    if not block:
        fehl("ww_xml_virtual_in_http() nicht gefunden.")
        return
    b = block.group(0)
    if b.count("$crlf") < 4:
    ...
    felder = re.findall(r"^\s*'([A-Z]+)'\s*=>\s*array\('([^']*)',\s*'(WW_FELD\.[A-Z_]+)'\)",
                        t, re.M)
    if len(felder) < 10:
    ...
    xml += ('<VirtualInHttp Title="Weissware 1" Comment="Test" '
...
def pruefe_rest(wurzel):
    ...
    for pfad in ("plugin.cfg", "release.cfg", "prerelease.cfg", "LICENSE", "README.md",
                 ...
                 "cron/cron.01min", "bin/weissware.py", "bin/dienst.sh",
                 ...
                 "webfrontend/htmlauth/index.php", "webfrontend/htmlauth/ww_test.php",
                 "webfrontend/html/index.php", "webfrontend/html/ww_lib.php"):
...
def pruefe_zusammenspiel(wurzel):
    py = (wurzel / "bin/weissware.py").read_text(encoding="utf-8")
    lib = (wurzel / "webfrontend/html/ww_lib.php").read_text(encoding="utf-8")
    ...
    lib_block = re.search(r"function ww_vorgaben.*?\n}", lib, re.S).group(0)
    ...
    for k in set(pv) & set(lv):
        ...
        b = lv[k].strip("'")
        if a != b:
            fehl("Vorgabewert %s: weissware.py sagt %s, ww_lib.php sagt %s" % (k, a, b))
    nur_py = set(pv) - set(lv)
    nur_php = set(lv) - set(pv) - {"aktionstoken", "wartezeit"}
    if nur_py:
        fehl("Vorgabewerte nur in weissware.py: %s" % sorted(nur_py))
    if nur_php:
        fehl("Vorgabewerte nur in ww_lib.php: %s" % sorted(nur_php))
    ...
    m = re.search(r"\$ww_schaltend = array\((.*?)\);", ep, re.S)
    ...
    mq = set(re.findall(r"'geraetN/([a-z_]+)'", lib))
    ...
    for aktion, funktion in (("WEISSWARE", "ww_status_felder"),
                             ("VERBRAUCH", "ww_verbrauch_felder")):
...
def pruefe_bausteinnamen(wurzel):
    ...
    den die Feldliste von VW und Weissware gar nicht kennt (nur Skoda hat ihn). Wer
